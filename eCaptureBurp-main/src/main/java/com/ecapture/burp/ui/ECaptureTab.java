...
import com.ecapture.burp.export.TrafficJsonExporter;
...
public class ECaptureTab {
    ...
    private JPanel createTopPanel() {
        ...
        exportButton.setToolTipText("Export captured HTTP traffic for AI analysis");
        exportButton.addActionListener(e -> exportTraffic());
    ...
    private void exportTraffic() {
        List<MatchedHttpPair> pairs = eventManager.getMatchedPairs();
        if (pairs.isEmpty()) {
            JOptionPane.showMessageDialog(mainPanel, "No captured traffic to export.",
        ...
        fileChooser.setSelectedFile(new File("ecapture-traffic-" + timestamp + ".json"));
        fileChooser.setDialogTitle("Export captured traffic for AI analysis");
        ...
        if (selectedFile.exists()) {
            int overwrite = JOptionPane.showConfirmDialog(mainPanel,
                    ...
                    "Export JSON", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                TrafficJsonExporter.write(outputPath, pairs);
                return null;
            }
            ...
            protected void done() {
                try {
                    get();
                    logging.logToOutput("Exported " + pairs.size() + " HTTP pairs to " + outputPath);
                    JOptionPane.showMessageDialog(mainPanel,
                            "Exported " + pairs.size() + " HTTP pairs to:\n" + outputPath,
                            "Export complete", JOptionPane.INFORMATION_MESSAGE);
                } catch (Exception ex) {
                    logging.logToError("Failed to export traffic: " + ex.getMessage());
                    JOptionPane.showMessageDialog(mainPanel,
                            "Failed to export traffic:\n" + ex.getMessage(),
